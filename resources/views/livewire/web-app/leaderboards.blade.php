    ...
    <div class="container mx-auto px-4 sm:px-6 md:px-8 py-4 sm:py-6 md:py-8 w-full">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 lg:gap-8">
            ...
            <div class="lg:col-span-8">
                <!-- Tabs -->
                <div class="flex mb-6">
                    <form id="tabForm" action="{{ route('leaderboards') }}" method="GET" class="flex w-full">
                        <input type="hidden" name="time_filter" value="{{ $timeFilter }}">
                        <input type="hidden" name="program_filter" value="{{ $programFilter }}">
                        
                        <button type="submit" name="tab" value="earners" 
                            class="sm:px-6 py-2 px-3 sm:py-3 border text-sm sm:text-md border-gray-700 transition-all flex-grow sm:flex-grow-0 {{ $activeTab == 'earners' ? 'bg-opacity-10 bg-[#00BFFF] text-[#00BFFF] border-[#00BFFF]' : 'text-gray-400' }} rounded-l-full font-semibold">
                            Top Earners
                        </button>
                        <button type="submit" name="tab" value="learners" 
                            class="sm:px-6 py-2 px-2 sm:py-3 border text-sm sm:text-md border-gray-700 transition-all flex-grow sm:flex-grow-0 {{ $activeTab == 'learners' ? 'bg-opacity-10 bg-[#00BFFF] text-[#00BFFF] border-[#00BFFF]' : 'text-gray-400' }} rounded-r-full font-semibold">
                            Top Learners
                        </button>
                    </form>
                </div>
                ...
                <div class="flex flex-col sm:flex-row gap-3 mb-4 sm:mb-6">
                    <form id="filterForm" action="{{ route('leaderboards') }}" method="GET" class="flex flex-col sm:flex-row gap-3 w-full">
                        ...
                        <select name="program_filter" onchange="this.form.submit()" 
                            class="bg-black bg-opacity-30 outline-none border border-blue-600 rounded-2xl px-3 sm:px-4 py-2 sm:py-3 w-full appearance-none focus:outline-none text-white cursor-pointer bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%2020%2020%22%3E%3Cpath%20d%3D%22M9.293%2012.95l.707.707L15.657%208l-1.414-1.414L10%2010.828%205.757%206.586%204.343%208z%22%20fill%3D%22%23fff%22%2F%3E%3C%2Fsvg%3E')] bg-no-repeat bg-[position:calc(100%-15px)_50%] bg-[length:20px_20px] pr-10 text-sm sm:text-base">
